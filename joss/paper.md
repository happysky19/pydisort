...
The original DISORT algorithm was implemented in Fortran [@stamnes1988numerically]. It has been widely used in Earth and planetary atmospheres (e.g. [@clough2005atmospheric; @li2018high; @tan2021atmospheric; @komacek2022patchy; @lee2024testing; @zhang2015aerosol]). However, the original Fortran implementation is limited by static memory allocation, which requires the user to specify the number of atmospheric layers and radiation streams at compile time. This limitation persists even with efforts to wrap the original Fortran code via `f2py` for more user-friendly access (e.g. [@chanGimeno](https://github.com/chanGimeno/pyDISORT); [@SeregaOsipov](https://github.com/SeregaOsipov/pyDISORT); [@danielkoll](https://github.com/danielkoll/PyDISORT3); [@mjwolff](https://github.com/mjwolff/pyDISORT)). Another approach is to implement the DISORT algorithm in pure Python, which solves the problem of static memory allocation but is less efficient than the Fortran implementation (e.g. [@ho2024pythonicdisort]).

An endeavor to overcome the limitations of the original Fortran implementation with even better efficiency is the `cdisort` library, which is a C reimplementation of DISORT[@buras2011new]. It preserves its stable, well-tested numerical core while adding portability and ease of embedding in modern workflows. The biggest break from the Fortran code is the use of dynamic memory allocation so that no compile-time parameters are required. It's a 100% double precision package that steers away from the occasional erroneous numerical spikes due to the mixed precision calculations in the original Fortran code. Additionally, it achieves a significant speedup when the scale of the problem is large enough (over 20x faster) due to the improved intensity correction method and the use of the Operation System's background memory management (compared to nested loops in Fortran DISORT for all multidimensional arrays).
...
3. achieves the same level of performance as the original `C/Fortran` implementation on the same hardware, with the extended capability of GPU acceleration in the future.
